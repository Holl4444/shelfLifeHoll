...
test('test with environment variables for sensitive data', async ({
  page,
}) => {
  ...
  if (!email || !password) {
    throw new Error(
      'Environment variables TEST_EMAIL or TEST_PASSWORD are not set'
    );
  }
